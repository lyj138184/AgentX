...
public class DocumentTree {
    ...
    private ProcessedSegment createSegmentFromNode(HeadingNode node) {
        ...
        // 添加特殊节点
        for (org.xhy.domain.rag.model.SpecialNode specialNode : node.getSpecialNodes()) {
    ...
    private void collectSpecialNodesFromChildren(HeadingNode node, ProcessedSegment segment) {
        for (HeadingNode child : node.getChildren()) {
            for (org.xhy.domain.rag.model.SpecialNode specialNode : child.getSpecialNodes()) {
                segment.addSpecialNode(specialNode);
            }
            // 递归处理子节点
    ...
    private void copySpecialNodes(ProcessedSegment source, ProcessedSegment target) {
        if (source.getSpecialNodes() != null && !source.getSpecialNodes().isEmpty()) {
            for (org.xhy.domain.rag.model.SpecialNode specialNode : source.getSpecialNodes().values()) {
