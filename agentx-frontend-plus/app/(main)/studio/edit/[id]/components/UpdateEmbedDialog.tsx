...
import { AgentEmbed, UpdateEmbedRequest } from "@/types/embed";
import { updateEmbedWithToast } from "@/lib/agent-embed-service";

interface UpdateEmbedDialogProps {
  open: boolean;
  onClose: () => void;
  agentId: string;
  embed: AgentEmbed;
...
export function UpdateEmbedDialog({ open, onClose, agentId, embed, models, onSuccess }: UpdateEmbedDialogProps) {
  const [formData, setFormData] = useState<UpdateEmbedRequest>({
    embedName: '',
    embedDescription: '',
    modelId: '',
  ...
  useEffect(() => {
    if (embed) {
      setFormData({
        ...
        modelId: embed.model?.id || '',
        providerId: embed.provider?.id,
        allowedDomains: embed.allowedDomains,
        dailyLimit: embed.dailyLimit,
        enabled: embed.enabled,
      });
      setDomainsText(embed.allowedDomains.join('\n'));
    }
  }, [embed]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const requestData: UpdateEmbedRequest = {
        ...formData,
        allowedDomains: domains,
      };

      const response = await updateEmbedWithToast(agentId, embed.id, requestData);
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>编辑嵌入配置</DialogTitle>
          <DialogDescription>
            修改 "{embed?.embedName}" 的配置
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="grid gap-4 py-4">
          <div className="flex items-center space-x-2">
            ...
            />
            <Label htmlFor="enabled">启用此嵌入配置</Label>
          </div>
